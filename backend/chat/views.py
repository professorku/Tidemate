...
def get_message_or_response(message_id):
    ...
    if not message:
        return None, Response({'detail': 'Message not found.'}, status=status.HTTP_404_NOT_FOUND)
...
class StartDirectConversationView(APIView):
    ...
    def post(self, request):
        target_user, error_response = get_target_user_or_response(request.data.get('user_id'))
        if error_response:
            return error_response

        existing_conversation = get_direct_conversation_between_users(request.user, target_user)
...
class ConversationMessagesView(generics.ListCreateAPIView):
    ...
    def _get_conversation_or_response(self):
        return get_conversation_or_response(self.request.user, self.kwargs['conversation_id'])
    ...
    def create(self, request, *args, **kwargs):
        ...
        message = send_message(conversation=conversation, sender=request.user, serializer=serializer)
...
class ConversationDeleteView(APIView):
    ...
    def delete(self, request, conversation_id):
        conversation, error_response = get_conversation_or_response(request.user, conversation_id)
        if error_response:
            return error_response

        try:
            delete_conversation(conversation=conversation, actor=request.user)
        except PermissionError as exc:
            return Response({'detail': str(exc)}, status=status.HTTP_403_FORBIDDEN)
        except ValueError as exc:
            return Response({'detail': str(exc)}, status=status.HTTP_400_BAD_REQUEST)
